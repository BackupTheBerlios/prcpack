...
// End of Spell Cast Hook
    ...
    int nHD = d4(GetCasterLevel(OBJECT_SELF) + GetChangesToCasterLevel(OBJECT_SELF)); //Roll to see how many HD worth of creature will be killed
    ...
    {
        nHD = 4 * (GetCasterLevel(OBJECT_SELF) + GetChangesToCasterLevel(OBJECT_SELF));//Damage is at max
    }
    ...
    while ((nHD > 0) && (bContinueLoop))
    {
        ...
        if(bContinueLoop == TRUE)
        {
            ...
            if(!MyPRCResistSpell(OBJECT_SELF, oLowest, fDelay))
            {
                //Make a Fort Save versus death effects
                if(!MySavingThrow(SAVING_THROW_FORT, oLowest, (GetSpellSaveDC() + GetChangesToSaveDC(OBJECT_SELF)), SAVING_THROW_TYPE_DEATH, OBJECT_SELF, fDelay))
                {
