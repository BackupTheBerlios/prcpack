...
void HenchDetermineSpecialBehavior(object oIntruder = OBJECT_INVALID, int bForceInterrupt = FALSE);
...
int HenchBashDoorCheck(object oIntruder = OBJECT_INVALID);
...
void SetEnemyLocation(object oEnemy)
{
    ...
    object oInvisTarget = CreateObject(OBJECT_TYPE_PLACEABLE, "plc_invisobj", enemyLocation);
...
void HenchDetermineSpecialBehavior(object oIntruder = OBJECT_INVALID, int bForceInterrupt = FALSE)
{
    object oTarget = GetNearestSeenOrHeardEnemyNotDead(HENCH_MONSTER_DONT_CHECK_HEARD_MONSTER);
...
int HenchTalentBardSong()
{
    ...
    if (GetHasSpellEffect(EFFECT_TYPE_SILENCE))
    {
        return FALSE;
    }
...
float GetEnemyChallenge(object oRelativeTo=OBJECT_SELF)
{
    ...
    return (log(fChallenge)/log(1.5))-IntToFloat(GetHitDice(oRelativeTo));
}
...
int HenchBashDoorCheck(object oIntruder = OBJECT_INVALID)
{
    ...
    if(GetIsObjectValid(oDoor))
    {
        ...
        if(bDoor == FALSE)
        {
            ...
            VoiceCuss();
            ActionDoCommand(HenchEquipAppropriateWeapons(OBJECT_SELF, -5., FALSE));
        }
    }
    return bDoor;
}
