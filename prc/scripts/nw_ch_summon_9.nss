...
//:: Edited By: Stratovarius, 2003-12-15: Added check for Augment Summoning
...
void main()
{
    ...
    //Added by Stratovarius
    // Applies the Augment Summoning Feat
    if ( GetHasFeat( 2510, GetFirstPC()))
...

    {
        ...
        eEffect = EffectDamageIncrease(1);
        ApplyEffectToObject(DURATION_TYPE_PERMANENT, eEffect, OBJECT_SELF);
        eEffect = EffectAbilityIncrease(ABILITY_CONSTITUTION, 4);
        ApplyEffectToObject(DURATION_TYPE_PERMANENT, eEffect, OBJECT_SELF);
        //SendMessageToPC(GetFirstPC(), "The If Statement is Triggered");
