  ...
  //Declare major variables
    ...
    while(GetIsObjectValid(oTarget))
    {
            if (spellsIsTarget(oTarget,SPELL_TARGET_STANDARDHOSTILE,oCreator))
            {
                ...
                //Make SR check
                if (!MyPRCResistSpell(oCreator, oTarget))
                {
                    nDamage = d8(nDice);
                    ...
                    nDamage = GetReflexAdjustedDamage(nDamage, oTarget, (GetSpellSaveDC()  + GetChangesToSaveDC(oCreator)), SAVING_THROW_TYPE_SONIC, oCreator);
