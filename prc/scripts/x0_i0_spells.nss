...
//:: changed all instances of GetCasterLevel to GetCasterLevel + GetChangesToCasterLevel
...
// modify spellsDispelAoE for Magic Circle vs Evil (Fist of Raziel)
...
void spellsInflictTouchAttack(int nDamage, int nMaxExtraDamage, int nMaximized, int vfx_impactHurt, int vfx_impactHeal, int nSpellID)
{
    ...
    int nExtraDamage = GetCasterLevel(OBJECT_SELF) + GetChangesToCasterLevel(OBJECT_SELF); // * figure out the bonus damage
        ...
        )
{
    ...
    while(GetIsObjectValid(oTarget))
    {
        if (spellsIsTarget(oTarget, nTargetType, oCaster) == TRUE)
        {
            ...
            {
                nResistSpellSuccess = MyPRCResistSpell(oCaster, oTarget);
            }
          if(!nResistSpellSuccess)
          {
                ...
                {
                  nSavingThrowSuccess = MySavingThrow(nSavingThrowType, oTarget, (GetSpellSaveDC() + GetChangesToSaveDC(oCaster)), nSavingThrowSubType);
                }
...
void spellsDispelAoE(object oTargetAoE, object oCaster, int nCasterLevel)
{
    ...
    SendMessageToPC(GetFirstPC()," AOE DISPELL");

    if (nId==SPELL_SU_CIRCEVIL) return;
    ...
    {
        if ( (GetStringRight( GetTag(oTargetAoE),5)!="NODIS"))
        {
