...
// End of Spell Cast Hook
    ...
    while ((nHD > 0) && (bContinueLoop))
    {
        ...
        if(oLowest != OBJECT_INVALID)
        {
            ...
            //Make SR check
            if (!MyPRCResistSpell(OBJECT_SELF, oLowest))
            {
                //Make Fort save
                if(!MySavingThrow(SAVING_THROW_WILL, oLowest, (GetSpellSaveDC() + GetChangesToSaveDC(OBJECT_SELF)), SAVING_THROW_TYPE_MIND_SPELLS))
                {
