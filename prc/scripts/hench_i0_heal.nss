...
int TalentHealNearestAlly(int iHasAlly, int iCurEffects, int bForce)
{
    ...
    if(iHasAlly)
    {
        ...
        int i = 0;
        object oMaster = GetMasterNPC(OBJECT_SELF);
        ...
        while (GetIsObjectValid(oTarget) && GetDistanceToObject(oTarget) < 20.0)
        {
            ...
            nAssocType = GetAssociateTypeNPC(oTarget);
            // note : ignore dominated creatures
            if (nAdjusted < nBase && oTarget != OBJECT_SELF && oTarget != oMaster &&
                !GetIsDead(oTarget) && GetAssociateTypeNPC(oTarget) != ASSOCIATE_TYPE_DOMINATED)
            {
...
int HenchTalentHeal(int iCurEffects, int bForce)
{
    ...
    if((GetAssociateHealMasterNPC() || bForce) && !GetIsDead(oRealMaster))
    {
...
int TalentCureConditionAllAllies(int iAlly)
{
    ...
    if(iAlly)
    {
        ...
        while(GetIsObjectValid(oTarget))
        {
            nSum = 0;
            nDispelSum = 0;
            if(!GetIsDead(oTarget) && GetIsFriend(oTarget) && GetAssociateTypeNPC(oTarget) != ASSOCIATE_TYPE_DOMINATED)
            {
            ...
            else if (!GetIsDead(oTarget) && !GetIsFriend(oTarget) && GetAssociateTypeNPC(oTarget) == ASSOCIATE_TYPE_DOMINATED &&
                (GetMasterNPC(OBJECT_SELF) == oTarget || GetMasterNPC(OBJECT_SELF) == GetMaster(oTarget) || GetFactionEqual(oTarget, OBJECT_SELF)))
            {
                eEffect = GetFirstEffect(oTarget);
                while(GetIsEffectValid(eEffect))
                {
