...
SetLocalInt(OBJECT_SELF, "X2_L_LAST_SPELLSCHOOL_VAR", SPELL_SCHOOL_TRANSMUTATION);
    ...
    //--------------------------------------------------------------------------
    int nDuration = 10 + (GetCasterLevel(OBJECT_SELF)+ GetChangesToCasterLevel(OBJECT_SELF));
    if (nDuration < 1)
    {
        nDuration = 10;
    }
    ...
    // Setup Effects
    //--------------------------------------------------------------------------
    effect eDam      = EffectDamage(nDamage, ChangedElementalDamage(OBJECT_SELF, DAMAGE_TYPE_FIRE));
...
void RunCombustImpact(object oTarget, object oCaster, int nLevel, int nMetaMagic)
{
    ...
    if (GetIsDead(oTarget) == FALSE)
    {
        ...
        if(!MySavingThrow(SAVING_THROW_REFLEX, oTarget, nDC, SAVING_THROW_TYPE_FIRE))
        {
            ...
            effect eDmg = EffectDamage(nDamage,ChangedElementalDamage(OBJECT_SELF, DAMAGE_TYPE_FIRE));
