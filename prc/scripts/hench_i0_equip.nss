...
int GetWeaponState(int nCondition)
{
    int nStatus = GetLocalInt(OBJECT_SELF, "HENCH_AI_WEAPON");
    return nStatus & nCondition ? TRUE : FALSE;
}
...
void EquipMeleeWeapons(object oTarget, int bIndicateStatus, int iCallNumber = 1)
{
    ...
    if (((dualWieldState == 0) && !GetHasFeat(FEAT_TWO_WEAPON_FIGHTING)) || (dualWieldState == 2))
    {
...
void EquipRangedWeapon(object oTarget, int bIndicateStatus, int iCallNumber = 1)
{
    ...
    if (!GetWeaponRanged(oRight))
    {
        ...
        {
            DelayCommand(iCallNumber == 1 ? 0.1 : (iCallNumber == 2 ? 0.4 : 1.5),
                EquipRangedWeapon(oTarget, iCallNumber + 1));
        }
        else
        {
            object oRealMaster = GetRealMaster();
...
void HenchEquipAppropriateWeapons(object oTarget, float fThresholdDistance, int bIndicateSwitch)
{
    int nWeaponStatus = GetInitWeaponStatus();
    int bUseRanged;
    ...
    if (nWeaponStatus & HENCH_AI_HAS_RANGED_WEAPON)
    {
        // has ranged weapons
        object oRealMaster = GetRealMaster();
        if(GetIsObjectValid(oRealMaster) && GetIsPC(oRealMaster) && !GetAssociateStateNPC(NW_ASC_USE_RANGED_WEAPON))
        {
            bUseRanged = FALSE;
        }
        else
    ...
    if (bIndicateSwitch && GetAssociateStateNPC(NW_ASC_USE_RANGED_WEAPON))
    {
        if (!(nWeaponStatus & HENCH_AI_HAS_RANGED_WEAPON))
        {
