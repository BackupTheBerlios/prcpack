...
int HenchTalentSummonAllies(int nForce = FALSE)
{
    ...
    if(!GetIsObjectValid(GetAssociateNPC(ASSOCIATE_TYPE_SUMMONED))
...
int HenchTalentUseProtectionOthers(object oDefault = OBJECT_INVALID, int iClearActions = TRUE)
{
    ...
    if (oDefault == OBJECT_INVALID) {
       ...
       if(GetAssociateStateNPC(NW_ASC_AGGRESSIVE_BUFF) && GetAssociateStateNPC(NW_ASC_HAVE_MASTER))
       {
           oTarget = GetMasterNPC();
       }
