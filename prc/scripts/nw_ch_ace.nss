...
//    Jug_Debug(GetName(OBJECT_SELF) + " is blocked by " + GetName(oDoor));
        ...
        && !GetLocalInt(OBJECT_SELF,"Scouting") && !IsOnOppositeSideOfDoor(oDoor, oRealMaster, OBJECT_SELF))
    {
        ClearAllActions();
        ClearForceOptions();
        if (GetAssociateTypeNPC(OBJECT_SELF) == ASSOCIATE_TYPE_HENCHMAN && (GetIsObjectValid(GetLocalObject(OBJECT_SELF, "LastTarget")) || GetLocalInt(OBJECT_SELF, "LastSeenOrHeard")))
        {
