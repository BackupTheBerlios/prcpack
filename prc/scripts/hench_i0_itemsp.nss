...
// remains in effect.
void InitializeItemSpells(int bNotBuffOnly = TRUE)
{
    itemSpellsFound = FALSE;
    onlyUseTalents = FALSE;
    itemTalentSpellCount = 0;
    bGlobalNoInventoryChange = GetAssociateTypeNPC(OBJECT_SELF) != ASSOCIATE_TYPE_HENCHMAN;

    nGlobalMeleeAttackers = GetNumberOfMeleeAttackers();

    int bSpellCastingProblem = InitializeSpellCasting();
    int bSpellFailure = GetHasAnyEffect2(EFFECT_TYPE_SILENCE, EFFECT_TYPE_SPELL_FAILURE);
    if (bSpellCastingProblem || bSpellFailure)
    {
        if (bNotBuffOnly)
        {
...
void ActionCastFixedSpellOnObject(int nSpell, object oTarget)
{
    ...
    if (bUseCheat)
    {
        SetLocalInt(OBJECT_SELF, "AI_CHEAT_CASTING", TRUE);
    }
    else
    {
        DeleteLocalInt(OBJECT_SELF, "AI_CHEAT_CASTING");
    }
    ActionCastSpellAtObject(nSpell, oTarget, METAMAGIC_ANY, bUseCheat);
}
...
void ActionCastFixedSpellAtLocation(int nSpell, location loc)
{
    ...
    ActionCastSpellAtLocation(nSpell, loc, METAMAGIC_ANY, bUseCheat);
}
