...
int GetMeleeAttackBonus(object oTarget = OBJECT_SELF)
{
    ...
    if(GetHasFeat(FEAT_WEAPON_FINESSE) && (nDexMod > nStrMod) && GetIsObjectValid(oWeaponRight))
    {
        nReturn += nDexMod;
    }
    else
...
int HenchTalentMeleeAttack(object oIntruder, float fThresholdDistance, int iMeleeAttackers, int iCreatureType)
{
    ...
    HenchEquipAppropriateWeapons(oTarget, fThresholdDistance, iCreatureType == 1);
...
//        }
    ...
    {
        relativeChallenge = iCreatureType == 1 ? IntToFloat(GetHitDice(OBJECT_SELF)) : GetChallengeRating(OBJECT_SELF);
        relativeChallenge -= GetAssociateTypeNPC(OBJECT_SELF) != ASSOCIATE_TYPE_HENCHMAN &&
            GetAssociateTypeNPC(OBJECT_SELF) != ASSOCIATE_TYPE_NONE ?
            IntToFloat(GetHitDice(oTarget)) : GetChallengeRating(oTarget);
    }
...
//  TODO      if (GetIsPlayableRacialType(OBJECT_SELF) || GetTag(OBJECT_SELF) == "x2_hen_valen")
//          bPlayableRace = TRUE;

        int bNumberofAttackers = WhirlwindGetNumberOfMeleeAttackers(WHIRL_DISTANCE);
        if (/*bPlayableRace &&*/ bNumberofAttackers)
        {
