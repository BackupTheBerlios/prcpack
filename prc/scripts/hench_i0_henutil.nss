...
void EquipItems(object oSource)
{
  ...
  while (GetIsObjectValid(oItem)) {
    if (!GetLocalInt(oItem,"DestroyItem")) {
      TalentIdentify(oItem);
      switch (GetBaseItemType(oItem)) {

      case BASE_ITEM_ARMOR:
        EquipIfWorthIt(oItem,INVENTORY_SLOT_CHEST, sHenchEquipArmor);
        break;
      ...
      case BASE_ITEM_CLOAK:
        EquipIfWorthIt(oItem,INVENTORY_SLOT_CLOAK, sHenchEquipCloak);
        break;

      case BASE_ITEM_BOOTS:
        EquipIfWorthIt(oItem,INVENTORY_SLOT_BOOTS, sHenchEquipBoots);
        break;

      case BASE_ITEM_BRACER:
      case BASE_ITEM_GLOVES:
        EquipIfWorthIt(oItem,INVENTORY_SLOT_ARMS, sHenchEquipHandwear);
        break;

      case BASE_ITEM_HELMET:
        EquipIfWorthIt(oItem,INVENTORY_SLOT_HEAD, sHenchEquipHelmet);
        break;

      case BASE_ITEM_BELT:
        EquipIfWorthIt(oItem,INVENTORY_SLOT_BELT, sHenchEquipBelt);
        break;

      case BASE_ITEM_RING:
        EquipIfWorthIt(oItem,INVENTORY_SLOT_LEFTRING, sHenchEquipRing, INVENTORY_SLOT_RIGHTRING);
        break;

      case BASE_ITEM_AMULET:
        EquipIfWorthIt(oItem,INVENTORY_SLOT_NECK, sHenchEquipAmulet);
        break;
...
void CopyFamiliarAnimalCompanionItems(object oPC)
{
    ...
    oAssociate = GetAssociateNPC(ASSOCIATE_TYPE_FAMILIAR, oPC);
    if (GetIsObjectValid(oAssociate))
    {
        CopyAssociateItems(oPC, oAssociate);
    }
    oAssociate = GetAssociateNPC(ASSOCIATE_TYPE_ANIMALCOMPANION, oPC);
    if (GetIsObjectValid(oAssociate))
    {
        CopyAssociateItems(oPC, oAssociate);
    }
...
// and destroying them to prevent them going hostile.
    if (GetIsObjectValid(GetAssociateNPC(ASSOCIATE_TYPE_FAMILIAR, oCreature)))
    {
        effect eUnSum = EffectVisualEffect(VFX_FNF_SMOKE_PUFF);
        location lUnSum = GetLocation(GetAssociateNPC(ASSOCIATE_TYPE_FAMILIAR, oCreature));
        ApplyEffectAtLocation(DURATION_TYPE_INSTANT, eUnSum, lUnSum, 5.0);
        DestroyObject(GetAssociateNPC(ASSOCIATE_TYPE_FAMILIAR, oCreature));
    }
    if (GetIsObjectValid(GetAssociateNPC(ASSOCIATE_TYPE_SUMMONED, oCreature)))
    {
        effect eUnSum = EffectVisualEffect(VFX_FNF_SMOKE_PUFF);
        location lUnSum = GetLocation(GetAssociateNPC(ASSOCIATE_TYPE_SUMMONED, oCreature));
        ApplyEffectAtLocation(DURATION_TYPE_INSTANT, eUnSum, lUnSum, 5.0);
        DestroyObject(GetAssociateNPC(ASSOCIATE_TYPE_SUMMONED, oCreature));
    }
    if (GetIsObjectValid(GetAssociateNPC(ASSOCIATE_TYPE_ANIMALCOMPANION, oCreature)))
    {
        effect eUnSum = EffectVisualEffect(VFX_FNF_SMOKE_PUFF);
        location lUnSum = GetLocation(GetAssociateNPC(ASSOCIATE_TYPE_ANIMALCOMPANION, oCreature));
        ApplyEffectAtLocation(DURATION_TYPE_INSTANT, eUnSum, lUnSum, 5.0);
        DestroyObject(GetAssociateNPC(ASSOCIATE_TYPE_ANIMALCOMPANION, oCreature));
    }
    if (GetIsObjectValid(GetAssociateNPC(ASSOCIATE_TYPE_DOMINATED, oCreature)))
    {
        effect eUnSum = EffectVisualEffect(VFX_FNF_SMOKE_PUFF);
        location lUnSum = GetLocation(GetAssociateNPC(ASSOCIATE_TYPE_DOMINATED, oCreature));
        ApplyEffectAtLocation(DURATION_TYPE_INSTANT, eUnSum, lUnSum, 5.0);
        DestroyObject(GetAssociateNPC(ASSOCIATE_TYPE_DOMINATED, oCreature));
    }
}
