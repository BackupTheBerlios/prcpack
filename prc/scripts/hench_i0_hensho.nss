...
//    Jug_Debug(GetName(OBJECT_SELF) + " res to shout of " + GetName(GetMaster()) + " index " + IntToString(nShoutIndex));
    ...
    if (GetAssociateStateNPC(NW_ASC_MODE_DYING))
    {
        return;
    }
    ...
    switch (nShoutIndex)
    {
        ...
        case ASSOCIATE_COMMAND_INVENTORY:
            if (GetHenchmanOptions(HENCH_HENAI_INVENTORY) & HENCH_HENAI_INVENTORY)
            {
                ...
                {
                    // * feedback as to why
                    SendMessageToPCByStrRef(GetMasterNPC(), 100895);
                }
            }
            else
            {
        ...
        case ASSOCIATE_COMMAND_ATTACKNEAREST: //Used to de-activate AGGRESSIVE DEFEND MODE
            {
                ...
                else
                {
                    ...
                    if (GetIsObjectValid(oLockedObject) && GetMasterNPC() == GetRealMaster() &&
                        ((GetAssociateTypeNPC(OBJECT_SELF) == ASSOCIATE_TYPE_FAMILIAR) ||
                    ...
                    else
                    {
                        ...
                        {
                            ActionForceFollowObject(GetRealMaster(), HenchGetFollowDistance());
                        }
                    }
                }
        ...
        case ASSOCIATE_COMMAND_MASTERUNDERATTACK:  //Check whether the master has you in AGGRESSIVE DEFEND MODE
            if(!GetAssociateStateNPC(NW_ASC_MODE_STAND_GROUND))
            {
                ...
                {
                    if(GetAssociateStateNPC(NW_ASC_MODE_DEFEND_MASTER) && GetIsObjectValid(GetLastHostileActor(oRealMaster)))
                    {
                        //PlayVoiceChat(VOICE_CHAT_ENEMIES);
                        HenchDetermineCombatRound(GetLastHostileActor(oRealMaster));
                    }
        ...
        case ASSOCIATE_COMMAND_HEALMASTER: //Ignore current healing settings and heal me now
            HenchResetHenchmenState();
            SetLocalObject(OBJECT_SELF, "HealTarget", GetMasterNPC());
            ExecuteScript("hench_o0_heal", OBJECT_SELF);
            break;

        case ASSOCIATE_COMMAND_PICKLOCK:
            OpenLock(GetLockedObject(GetMasterNPC()));
            break;

        case ASSOCIATE_COMMAND_DISARMTRAP:
            ForceTrap(GetNearestTrapToObject(GetMasterNPC()));
            break;
        ...
        case ASSOCIATE_COMMAND_LEAVEPARTY:
            {
                ...
                if(GetIsObjectValid(oMaster))
                {
                    ...
                    {
                        FireHenchman(GetMasterNPC(), OBJECT_SELF);
                    }
                }
                break;
            }
