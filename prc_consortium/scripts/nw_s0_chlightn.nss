...
// End of Spell Cast Hook
    ...
    if (spellsIsTarget(oFirstTarget, SPELL_TARGET_SELECTIVEHOSTILE, OBJECT_SELF))
    {
        ...
        //Make an SR Check
        if (!MyPRCResistSpell(OBJECT_SELF, oFirstTarget))
        {
            //Adjust damage via Reflex Save or Evasion or Improved Evasion
            nDamStrike = GetReflexAdjustedDamage(nDamage, oFirstTarget, (GetSpellSaveDC()+ GetChangesToSaveDC(OBJECT_SELF)), SAVING_THROW_TYPE_ELECTRICITY);
            //Set the damage effect for the first target
            eDamage = EffectDamage(nDamStrike, ChangedElementalDamage(OBJECT_SELF, DAMAGE_TYPE_ELECTRICAL));
    ...
    while (GetIsObjectValid(oTarget) && nCnt < nCasterLevel)
    {
        ...
        if (oTarget != oFirstTarget && spellsIsTarget(oTarget, SPELL_TARGET_SELECTIVEHOSTILE, OBJECT_SELF) && oTarget != OBJECT_SELF)
        {
            ...
            if (!MyPRCResistSpell(OBJECT_SELF, oTarget, fDelay))
            {
                ...
                //Adjust damage via Reflex Save or Evasion or Improved Evasion
                nDamStrike = GetReflexAdjustedDamage(nDamage, oTarget, (GetSpellSaveDC() + GetChangesToSaveDC(OBJECT_SELF)), SAVING_THROW_TYPE_ELECTRICITY);
                //Apply the damage and VFX impact to the current target
                eDamage = EffectDamage(nDamStrike /2, ChangedElementalDamage(OBJECT_SELF, DAMAGE_TYPE_ELECTRICAL));
