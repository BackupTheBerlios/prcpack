...
// End of Spell Cast Hook
    ...
    int nPool = 2* (GetCasterLevel(OBJECT_SELF) + GetChangesToCasterLevel(OBJECT_SELF));
       ...
       // if(GetIsObjectValid(oMaster) && GetIsEnemy(oMaster))
        {
               ...
               )
            {
                ...
                if (spellsIsTarget(oTarget, SPELL_TARGET_STANDARDHOSTILE, OBJECT_SELF))
                {
                    ...
                    if (!MyPRCResistSpell(OBJECT_SELF, oTarget) && !MySavingThrow(SAVING_THROW_WILL, oTarget, nSpellDC))
                    {
