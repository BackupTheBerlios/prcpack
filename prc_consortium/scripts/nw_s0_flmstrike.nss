...
// End of Spell Cast Hook
  ...
  while ( GetIsObjectValid(oTarget) )
  {
       if (spellsIsTarget(oTarget, SPELL_TARGET_STANDARDHOSTILE, OBJECT_SELF))
       {
           ...
           if (!MyPRCResistSpell(OBJECT_SELF, oTarget, 0.6))
           {
                ...
                nDamage2 = GetReflexAdjustedDamage(nDamage/2, oTarget, (GetSpellSaveDC()+ GetChangesToSaveDC(OBJECT_SELF)), SAVING_THROW_TYPE_DIVINE);
                nDamage = GetReflexAdjustedDamage(nDamage/2, oTarget, (GetSpellSaveDC()+ GetChangesToSaveDC(OBJECT_SELF)), SAVING_THROW_TYPE_FIRE);
                ...
                eFire =  EffectDamage(nDamage, ChangedElementalDamage(OBJECT_SELF, DAMAGE_TYPE_FIRE));
                if(nDamage > 0)
                {
