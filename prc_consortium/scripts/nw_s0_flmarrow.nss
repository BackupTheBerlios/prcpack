    ...
    if(!GetIsReactionTypeFriendly(oTarget))
    {
        ...
        for (nCnt = 1; nCnt <= nMissiles; nCnt++)
        {
            if(!MyPRCResistSpell(OBJECT_SELF, oTarget, fDelay))
            {
                ...
                nDam = GetReflexAdjustedDamage(nDam, oTarget, (GetSpellSaveDC()+ GetChangesToSaveDC(OBJECT_SELF)), SAVING_THROW_TYPE_FIRE);
                //Set damage effect
                effect eDam = EffectDamage(nDam, ChangedElementalDamage(OBJECT_SELF, DAMAGE_TYPE_FIRE));
