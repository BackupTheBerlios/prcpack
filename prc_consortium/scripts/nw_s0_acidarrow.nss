...
SetLocalInt(OBJECT_SELF, "X2_L_LAST_SPELLSCHOOL_VAR", SPELL_SCHOOL_CONJURATION);
   ...
   object oTarget = GetSpellTargetObject();
    ...
    int nDuration = ((GetCasterLevel(OBJECT_SELF) + GetChangesToCasterLevel(OBJECT_SELF))/3);
    ...
    if (GetIsReactionTypeFriendly(oTarget) == FALSE)
    {
        ...
        if(MyPRCResistSpell(OBJECT_SELF, oTarget) == FALSE)
        {
            //----------------------------------------------------------------------
            // Do the initial 3d6 points of damage
            //----------------------------------------------------------------------
            int nDamage = MaximizeOrEmpower(6,3,nMetaMagic);
            effect eDam = EffectDamage(nDamage, ChangedElementalDamage(OBJECT_SELF, DAMAGE_TYPE_ACID));
...
void RunImpact(object oTarget, object oCaster, int nMetaMagic)
{
    ...
    if (GetIsDead(oTarget) == FALSE)
    {
        ...
        effect eDam = EffectDamage(nDamage, ChangedElementalDamage(oCaster, DAMAGE_TYPE_ACID));
